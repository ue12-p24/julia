...
\begin{frame}[fragile]{Opérateur \texttt{::}}
	...
	Si l'argument n'est pas non plus typé, on utilise \jl{_} à la place pour indiquer un argument inutilisé :
	\begin{repl}
...
\begin{frame}[fragile]{Types composites}
	...
	Une définition de type ne peut pas être effectuée dans une fonction.
\end{frame}
...
\begin{frame}[fragile]{Types des types}
	...
	\begin{itemize}
		...
		\item \jl{DataType} pour tous les autres. Exemples : \jl{Int}, \jl{Dict{String,Int}}, \jl{Vector{String}}, \jl{Tuple{Float64,Float64}}.
	\end{itemize}
	\vfill
...
\begin{frame}[fragile]{Représentation mémoire}
	Dans l'exemple précédent, \texttt{0x00007f5208a90de0} est l'adresse de la chaîne \jl{"hic"}. Celle-ci est représentée en mémoire à cette adresse par les quatre caractères \jl{'h'}, \jl{'i'}, \jl{'c'} et \jl{'\0'} (qui indique la fin d'une chaîne de caractères) consécutifs.
