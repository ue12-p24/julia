...
\begin{frame}{Règle diagonale}
	\begin{definition}
		\noindent\emph{Règle diagonale} : si une variable de type n'apparaît en position covariante (c'est-à-dire comme paramètre direct de seulement \jl{Tuple} et d'\jl{Union}) au moins deux fois et jamais en position invariante, alors cette variable est \emph{diagonale} et ne peut représenter qu'un type \textbf{concret}.
	\end{definition}
	
	Exemples et contre-exemples :
	\begin{itemize}
		\item \jl{Tuple{T,T} where T} admet pour sous-type \jl{Tuple{Int,Int}} mais pas \jl{Tuple{Integer,Integer}} car \jl{Integer} n'est pas concret. On retiendra que \jl{x isa Tuple{T,T} where T} est bien vrai pour \jl{x = (3, 4)} mais pas pour \jl{x = ("foo", 14.5)}
		\vspace{0.4em}
...
\begin{frame}[fragile]{Objet appelable}
	...
	\begin{julia}
		...
		plus12 = PlusN(12)  # an instance of the custom struct
		plus12(8) # == 20   # the instance has been made callable
	\end{julia}

	Les fonctions usuelles ne sont qu'un cas particulier de cette constructions : chaque définition de fonction \jl{foo} revient à créer un type singleton de la forme \jl{struct var"typeof(foo)" end} et à ajouter des méthodes dessus.
\end{frame}
...
\begin{frame}[fragile]{Dispatch multiple}
	\begin{definition}
		...
		La signature d'un appel est toujours un type concret.
	\end{definition}
...
\begin{frame}[fragile]{Dispatch multiple}
	\begin{definition}
		...
		Le choix de la méthode à exécuter lors d'un appel dépend donc du type de \textbf{tous} les arguments. On parle de \emph{dispatch multiple}.
	\end{definition}
...
%	Exemple : on considère un type \jl{Attendance} qui représente la liste des élèves présents à une classe avec deux champs : \jl{students} pour la liste des noms d'élèves, et \jl{present} une liste booléenne indiquant si l'élève est présent ou non. Pour itérer seulement sur les noms des élèves présents, on peut déclarer :
	\begin{julia}
		struct Attendance
			students::Vector{String}  # list of student names
			present::Vector{Bool}     # true if the student is present
		end
		
		# let's iterate only on the students who are present
		function Base.iterate(att::Attendance, i=1)
			...
			return nothing # reached the end of the student list
		end
	\end{julia}
...
\begin{frame}[fragile]{Tableaux}
	...
	Si les indices utilisés sont des entiers consécutifs, le type mérite peut-être d'être déclaré comme un sous-type de \jl{AbstractArray}. Il est alors demandé de créer une méthode pour \jl{getindex(::T, ::Int)} spécifiquement, et une pour \jl{size(::T)}. L'interface complète est sur :
