...
\begin{center}
	...
	TD \no5 : hiérarchie des types et dispatch multiple
	\vspace{2em}
\end{center}
...
\begin{enumerate}[resume]
	...
	\item Implémenter la fonction \jl{Base.:(==)(t1::AbstractTree, t2::AbstractTree)} qui vérifie si deux arbres sont identiques.
...
\begin{enumerate}
	...
	\item Écrire un type \jl{BinaryTree} permettant d'implémenter un arbre binaire. Le type doit \underline{garantir} que l'arbre est binaire, et implémenter .
	
	Comme à l'exercice précédent, il est préférable d'ajouter une variable de type \jl{T} pour préciser le type des éléments de l'arbre.
	\item Écrire un constructeur externe pour \jl{BinaryTree} qui prend pour unique argument un vecteur non vide \jl{l}, et renvoie un arbre binaire tel que \jl{l[1]} soit la racine et pour tout indice \jl{i} de \jl{l}:
	\begin{itemize}
...
Implémenter le protocole d'itération sur \jl{LeafIterator} de façon à ce que la boucle \jl{for x in leaves(tree)} où \jl{tree isa AbstractTree} fasse passer \jl{x} par chaque feuille -- et non pas chaque élément -- de \jl{tree}.
...
%	\item Parmi les arbres représentés ci-après, trouver le seul qui soit un tas.
	...
	\item Créer un type \jl{Heap} sous-type de \jl{AbstractTree} qui représente le tas sous sa forme de vecteur.
	
	\item En utilisant les questions 3 et 5 de l'exercice précédent, créer un constructeur externe pour \jl{Heap} qui prend un \jl{BinaryTree} en entrée, et, réciproquement un constructeur externe pour \jl{BinaryTree} qui prend un \jl{Heap} en entrée.
	...
	\item Créer une méthode \jl{Base.pop!(h::Heap)} qui renvoie la racine du tas et qui la supprime tout en préservant la propriété du tas. Pour ce faire, l'algorithme consiste à supprimer le dernier élément \jl{x} et le mettre à la place de la racine, puis à faire descendre \jl{x} dans l'arbre en échangeant \jl{x} avec sa sous-branche de plus petite valeur, jusqu'à ce que \jl{x} soit inférieur à ses deux sous-branches. $(*)$ Quelle est sa complexité ?
	...
	\textit{Indice : on pourra utiliser les méthodes de \jl{push!} et \jl{pop!} définie précédemment.}
\end{enumerate}
...
\begin{itemize}
	...
	\item Si \jl{element == 1}, la racine a pour unique sous-nœud l'arbre inverse de Syracuse de racince 2. Sinon :
	\begin{itemize}
		\item si \jl{element-1} est divisible par 3 et que \jl{(element-1)÷3} est impair, alors la racine a deux sous-nœuds, qui sont les arbres inverse de Syracuse ayant pour racine \jl{2*element} et \jl{(element-1)÷3}
		\item sinon, la racine n'a qu'un seul sous-nœud, qui est l'arbre inverse de Syracuse ayant pour racine \jl{2*element}.
	\end{itemize}
\end{itemize}
...
\begin{enumerate}
	...
	\item Implémenter la fonction \jl{depthof(t::AbstractTree{T}, x::T) where T} qui calcule la distance entre \jl{x} et la racine de \jl{t}, c'est-à-dire le \textbf{plus petit} nombre de branches séparant la racine d'une apparition de \jl{x} dans l'arbre. En particulier, \jl{depthof(t, root(t)) == 0}.
	
	La fonction renvoie \jl{nothing} si \jl{x} n'apparaît pas dans \jl{t}.

	\item Soit $\jl{n}\in\mathbb N^*$. Comment exprimer le nombre de termes de la suite de Syracuse commençant par \jl{n} et se terminant au premier 1 en fonction du résultat de \jl{depthof(InverseSyracuseTree(1), n)} ?
	
	\item Montrer qu'il existe des couples d'entier $(\jl{n},\jl{m})\in(\mathbb N^*)^2$ tels que \jl{depthof(InverseSyracuseTree(n), m)} ne termine pas.
\end{enumerate}
